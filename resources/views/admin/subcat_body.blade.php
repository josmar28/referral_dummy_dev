<form method="POST" action="{{ asset('admin/subcat/add') }}">
    {{ csrf_field() }}
    <fieldset>
        <legend><i class="fa fa-hospital-o"></i> Sub Category</legend>
    </fieldset>
    <input type="hidden" value="@if(isset($data->id)){{ $data->id }}@endif" name="id">
    <input type="hidden" value="0" name="void">
    <div class="form-group">
        <label>Daignosis Mian Category Code:</label>
        <!-- <input type="text" class="form-control" value="@if(isset($data->diagmcat)){{ $data->diagmcat }}@endif" name="diagmcat" required> -->
